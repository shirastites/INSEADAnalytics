...
**Your Answers here:**
1) In the dataSet1.R file, on the 8th line, the code includes ```mytickers = c("SPY", "AAPL"), which indicates that the mytickers data is a matrix of data from both SPY and AAPL stocks. 

2) Cumulative returns = 596.4
   Average daily returns = 0.148
   Standard deviation of daily returns = 2.39 

3) You would have to insert the ticker code for yahoo into the ```mytickers= c("SPY"", "YHOO") in the dataSet1.R file and resubmit the requests with round(100*sum(StockReturns[,2]),2), etc...
...
**Your Answers here:**
1) You can use nrow(transposedData) and ncol(transposedData) to get 2 rows and 4029 columns.
2) portfolio = apply(StockReturns,c(1,2),mean)
   names(portfolio) <- rownames(StockReturns)
   pnl_plot(portfolio)
...
1) Change the start date to 12/29/2000 in dataSet1.R, reload the source("dataSet1.R"), and rerun the exercises
...
**Your Answers here:**
1) The sum(myData != StockReturns[1:20,]) returns a value of 20. 
2) The class of myData is a "data.frame", whereas the class of StockReturns is a "matrix". They cannot be added with a + operator as a result. Because the != operator is a logical statement and sum() can be used to sum logical statements, we get a 1 or TRUE response that is summed 20 times.
