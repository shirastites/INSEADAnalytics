...
crisis_data = futures_data[as.Date(rownames(futures_data)) < "2007-10-01" , ]
show_data = data.frame(colnames(crisis_data))
...
show_data = data.frame(cbind(colnames(crisis_data), round(cor(crisis_data),2)))
...
<br>
2) The highest weights of the first principal portfolio component is are from components 1 and 2 with eigenvalues of 14.04 and 7.82

<br>
3) You can classify the first 10 components as strong/stable financial futures. These can be interpreted as the cost of borrowing.

<br>
4) crisis_data = futures_data[as.Date(rownames(futures_data)) > “2007-10-01” & as.Date(rownames(futures_data)) < “2009-03-01”, ])

crisis_data = futures_data[as.Date(rownames(futures_data)) > “2002-11-01” & as.Date(rownames(futures_data)) < “2007-10-01”, ])

<br>

5) During the financial crisis years the relative significance of component 1 increased to around 17 from 14 in eigenvalue. In addition the pairwise correlations of staple futures is somewhat higher. Pre-crisis years, the eigenvalue of component 1 decreased to 11. In addition, up to 6 components make up 50% of the variance. More components make up the line before the "elbow" in the scree chart as well.    
...
1) The first line of code initializes the trading signal at 0. The next line defines a frequency term "MAfreq" that is a sequence of numbers starting at 10 and ending at 250 with intervals of 20. This is iterated from 1 to each of the numbers is the sequence of MAfreq (ie. 1:10, 1:30, 1:50, etc.), in which a signal is determined based on the sign (1,0, or -1) of the output of function(r) that is applied to the futures data. The next line then invests $1 dollar a day if the absolute value of r is not 0.It creates the returns in a table and then uses the signal from 2 days ago to determine the trading strategy.

<br>

2) Playing with the command line MAfreq<-seq(10,250,by=20), affects how often the trading strategy is implemented. If you manipulate the 20, the results change somewhat, particularly DD, and the average returns. Changing the values that frame the boundaries of the sequence more significantly affects the output. Increasing the 250 to 500 for example reduces returns from 6.2 to 4.9, and increases DD from 14.6 to 24.2.
