...
1. There are not any strong correlations (>0.7) between the responses, except for correlations between the same questions which are at 1.
2. This makes sense because the correlations that are perfect, 1, are comparing the same questions with each other. Only a few questions have correlations above 0.6, with others having correlations around 0.4-0.6.
...
1. The table and plot indicate the amount of variance that is explained by the data by varying the number of components or categories that capture the 29 variables. For example, you can see from the table that 50% of the variance is captured when there are 5 components. 80% of the variance is captured by using 17 components. The plot provides a visual to show that the majority of variance is captured with 5 components.
2. The plot has a decreasing slope because it is capturing sorted eigenvalues that show the most significant to least significant components. You can see that the slope decreases more gradually as the latter components are added because after a certain point increasing components do not add significantly to describe the data, they are more like "noise"
3. You would prefer to see a majority of the variance in the data being explained by a few variables. This would allow you to focus on fewer components and create a simpler analysis. You would also prefer to see a clear "elbow" in the plot that would help delineate the number of components to include. 
...
1. The first factor (component 1) captures the value of the boat as a symbol of status and power, the second factor (component 2) captures the value of the boat as adventure and escape.These do make business sense in that they capture different "needs"-based reasons for someone to buy a boat.  (Component 3 is trust/brand, component 4 is DIY/experienced buyer, component 5 is cheap and functional)
2. I ultimately decided to choose 5 factors for this data/customer base. a) captures more than 50% of the variance and additional components add minimally to this, b) in addition each of these 5 components capture significantly different qualities, and after 5 components each component draws from a single question rather than a series of questions, c) looking at more than 5 elements is complex/tedious/expensive and may result in customers that share multiple components. 
3. Component 1: Status and Power
   Component 2: Adventure
   Component 3: Brand  
   Component 4: Experience/DIY
   Component 5: Value 
4. I learned the function of the principal component analysis and how to use it within an analysis.
...
1. We have individuals who care a lot solely about brand, or secondarily about status and value. Other individuals care about status and value. Others prioritize adventure and value. Another cares solely about being experienced and DIY.
2. Component 1: Status and Power - Q9
   Component 2: Adventure - Q18 
   Component 3: Brand - Q4
   Component 4: Experience/DIY - Q11
   Component 5: Value - Q2
<hr>\clearpage
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(10,19,5,12,3) #c(10,19,5,12,3) 
...
1. The numbers in the last table indicate the relative "strangeness" of each segment/cluster from the average population with respect to each question. Negative numbers indicate that the average response of the segment is below the average population's response. A positive number indicates the average response of the segment is higher than that of the general population for that question.

2. Based on the tables and snake plot, we can see some similarities between segments 3 and 5, and segments 1, 2 and 4. For components 3 and 5, the response is generally higher than the general population. For components 1, 2 and 4, the response is closer to the general population, or slightly lower. 

Component 1: Status and Power  - Q11, performing repairs is strongly negative (-.41)    Component 2: Adventure - Q9, seeing boat as a status symbol is very negative (-.27)
Component 4: Experience/DIY - Q2, Price sensitivity over brand is very positive (.23)

Component 3: Brand - Q9, seeing boat as a status symbol is very positive (.39)
Component 5: Value - Q2, Price sensitivity over brand is very positive (.48)
...
1. There were 5 segments in our market. We selected these base on the 5 characteristics. See above.*
2.
Component 1: Status and Power  - Q11, performing repairs is strongly negative (-.41)    Component 2: Adventure - Q9, seeing boat as a status symbol is very negative (-.27)
Component 4: Experience/DIY - Q2, Price sensitivity over brand is very positive (.23)
